...
//Gstar gs-87协议解析后存放结构体
typedef struct  
{										    
 	u8 svnum;					//可见卫星数
	nmea_slmsg slmsg[20];		//最多20颗卫星
...
void NMEA_GPGSV_Analysis(nmea_msg *gpsx,u8 *buf);//分析GPGSV信息
void NMEA_GPGGA_Analysis(nmea_msg *gpsx,u8 *buf);
void NMEA_GPGSA_Analysis(nmea_msg *gpsx,u8 *buf);
void NMEA_GPRMC_Analysis(nmea_msg *gpsx,u8 *buf);//分析GPRMC信息
void NMEA_GPVTG_Analysis(nmea_msg *gpsx,u8 *buf);//分析GPVTG信息
