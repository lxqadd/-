...
nmea_msg gpsx; 	//GPS信息
__align(4) u8 dtbuf[50]; //缓冲区


extern g_TxBuf1[UART1_TX_BUF_SIZE];//串口1的发送缓冲区
extern g_TxBuf2[UART2_TX_BUF_SIZE];//串口2的发送缓冲区
extern g_RxBuf1[UART1_RX_BUF_SIZE];//串口1的接收缓冲区
...
/*
*管脚说明：
* 5u接电源（3.3V），GND接地，Tx-PA9,Rx-PA10
*/
...
{			 
	uint8_t ucTemp;		//GPS数据
	u16 len=0,i;
	u8 rval=0;
	while((ucTemp&0X8000)==0 && len<100)//等待接收到应答   
	{
		len++;
		bsp_DelayMS(5);
	}		 
	if(len<250)   	//超时错误.
	{
		len=ucTemp&0X7FFF;	//此次接收到的数据长度 
		for(i=0;i<len;i++)if(g_RxBuf2[i]==0XB5)break;//查找同步字符 0XB5
		if(i==len)rval=2;						//没有找到同步字符
		else if(g_RxBuf2[i+3]==0X00)rval=3;//接收到NACK应答
...
{
	u8 i;
	_ublox_cfg_cfg *cfg_cfg=(_ublox_cfg_cfg *)g_TxBuf2;
...
{
	_ublox_cfg_msg *cfg_msg=(_ublox_cfg_msg *)g_TxBuf2;
...
{
	_ublox_cfg_prt *cfg_prt=(_ublox_cfg_prt *)g_TxBuf2;
	...
	bsp_DelayMS(200);				//等待发送完成 
	//InitHardUart2(void);	//重新初始化串口2   
...
{
	_ublox_cfg_tp *cfg_tp=(_ublox_cfg_tp *)g_TxBuf2;
...
{
	_ublox_cfg_rate *cfg_rate=(_ublox_cfg_rate *)g_TxBuf2;
...
void Gps_Msg_Show(void)
{
	...
	LCD_DispStr(100,60,dtbuf,&tFont);

	tp=gpsx.latitude;	   
	sprintf((char *)dtbuf,"纬度:%.5f %1c   ",tp/=100000,gpsx.nshemi);	//得到纬度字符串
 	LCD_DispStr(100,80,dtbuf,&tFont);		 	 
		 	
	sprintf((char *)dtbuf,"UTC 日期:%04d/%02d/%02d   ",gpsx.utc.year,gpsx.utc.month,gpsx.utc.date);	//显示UTC日期
	LCD_DispStr(100,100,dtbuf,&tFont);			    
	sprintf((char *)dtbuf,"UTC 时间:%02d:%02d:%02d   ",gpsx.utc.hour+8,gpsx.utc.min,gpsx.utc.sec);	//显示UTC时间
	LCD_DispStr(100,120,dtbuf,&tFont);	
